...
class ConfigurableRobotController:
    """
    handles N robot arms - user picks how many
    """
    def __init__(self, num_arms: int, config: SimConfig, headless: bool = True, capture_frames: bool = False):
        if num_arms < 1:
            raise ValueError("need at least 1 arm")
        
        self.num_arms = num_arms
        
        # calculate belt size from arm count
        arm_spacing = 2.0
        ...
        logger.info(f"[SETUP] {num_arms} arms, belt length {config.belt_length}")
        logger.info(f"[SETUP] Cam scale: {perspective_scale_factor:.2f}, {top_scale_factor:.2f}")
        logger.info(f"[SETUP] Perspective cam: {config.perspective_camera_position}")
        logger.info(f"[SETUP] Top cam: {config.top_camera_position}")
        logger.info(f"[SETUP] Belt: start={config.conveyor_start_x:.2f}, end={config.conveyor_end_x:.2f}")
        ...
        # recycling bin at end of belt (blue one)
        ...
        logger.info(f"[SETUP] Recycling bin at X={recycling_bin_x:.2f}")
        ...
        logger.info(f"[SETUP] Arm positions: {arm_positions}")
    ...
    def run(self):
        ...
        while True:
            ...
            if frame_count % (50//min(self.num_arms,5)) == 0:
                ...
                logger.info(f"[SPAWN] New {obj_type}: {object_name} (ID: {object_id}), total: {num_objects}")
            ...
            for arm in self.arms:
                if arm['last_img'] is not None and arm['last_img'].shape[0] > 0:
                    try:
                        ...
                        if arm['id'] == self.spawn_side_arm['id']:
    ...
    def _handle_wait_for_object(self, arm, sim_time):
        """yolo detection - find target"""
        if arm['target_info'] is not None:
            logger.debug(f"[ARM {arm['id']}] Already has target")
            return
        ...
        if class_name not in self.config.trash_classes:
            logger.info(f"[ARM {arm['id']} DETECT] '{class_name}' recyclable - skip")
        ...
        logger.info(f"[ARM {arm['id']} DETECT] World pos: {detected_world_pos}")
        ...
        if detected_world_pos[0] > arm_detection_line_x:
            ...
            logger.debug(f"[ARM {arm['id']} DETECT] Tracking at X={detected_world_pos[0]:.2f}")

    def _handle_prepare_pick(self, arm, sim_time):
        """timing for pickup - when to go"""
        if arm['picked']:
            return
        ...
        logger.debug(f"[ARM {arm['id']}] {obj_name} - time={time_to_pickup:.2f}s (y={y:.2f})")
        
        if 0 < time_to_pickup <= self.config.arm_lead_time:
            logger.info(f"[ARM {arm['id']}] >>> STARTING PICK: {obj_name}")
            arm['arm_processing_trash'] = True
            arm['picking_start_step'] = self.simulation_step
            arm['state'] = ArmState.PICKING
        else:
            logger.debug(f"[ARM {arm['id']}] Waiting - {time_to_pickup:.2f}s away")

    def _handle_picking(self, arm):
        """pickup logic - actually grab"""
        if arm['picking_start_step'] and (self.simulation_step - arm['picking_start_step']) > arm['picking_timeout_steps']:
            ...
            logger.warning(f"[ARM {arm['id']}] Timeout after {steps_elapsed} steps")
            arm['picked'] = False
            arm['tracking'] = False
        ...
        if arm['arm_substate'] is None:
            logger.info(f"[ARM {arm['id']}] Moving to: {pickup_pos}")
            ...
            if wait_for_arm_to_reach(arm['kuka_id'], above_pos, threshold=self.config.arm_threshold):
                logger.info(f"[ARM {arm['id']}] Moving down: {pickup_pos}")
            ...
            if wait_for_arm_to_reach(arm['kuka_id'], pickup_pos, threshold=self.config.arm_threshold):
                ...
                if closest_obj_id is not None and closest_dist < 0.5:
                    logger.info(f"[ARM {arm['id']}] Grabbing ID {closest_obj_id} (dist={closest_dist:.2f})")
                    ...
                    logger.warning(f"[ARM {arm['id']}] No obj found (dist={closest_dist:.2f})")
                    arm['arm_substate'] = None
                    arm['picking_start_step'] = None
                    arm['target_info'] = None
                    arm['state'] = ArmState.RESETTING
    ...
    def _handle_lifting(self, arm):
        ...
        if arm['arm_substate'] is None:
            logger.info(f"[ARM {arm['id']}] Lifting")
            ...
            if wait_for_arm_to_reach(arm['kuka_id'], lift_pos, threshold=self.config.arm_threshold):
                logger.info(f"[ARM {arm['id']}] Moving to drop")
            ...
            if (time.time() - arm['drop_reached_time']) >= self.config.arm_pause_before_drop:
                logger.info(f"[ARM {arm['id']}] Releasing ID {arm['target_object_id']}")
                release_object(arm['constraint_id'])
                arm['release_time'] = time.time()
                
                if arm['target_info'] is not None and 'class_name' in arm['target_info']:
                    logger.info(f"[ARM {arm['id']}] Processed: {arm['target_info']['class_name']}")
    ...
    def _handle_resetting(self, arm):
        if arm['arm_substate'] is None:
            logger.info(f"[ARM {arm['id']}] Resetting joints")
            ...
            if wait_for_joints_to_reach(arm['kuka_id'], arm['initial_joint_positions'], threshold=0.1):
                logger.info(f"[ARM {arm['id']}] Reset done!")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Multi-arm sim')
    parser.add_argument('--num-arms', type=int, default=3, help='Number of arms (default: 3)')
    ...
    logger.info(f"[MAIN] Starting with {num_arms} arms")
    ...
    try:
        ...
        if controller.capture_frames:
            ...
            video_fps = int(controller.config.simulation_fps)
            ...
            logger.info("[VIDEO] Cleaning up frames...")
            ...
            for frame_dir in frame_dirs:
                if os.path.exists(frame_dir):
                    ...
                    logger.info(f"[VIDEO] Cleaned {len(frame_files)} frames from {os.path.basename(frame_dir)}")
