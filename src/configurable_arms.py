...
class ConfigurableRobotController:
    ...
    def run(self):
        ...
        while True:
            ...
            # spawn objects
            if frame_count % (40//min(self.num_arms,5)) == 0:
            ...
            for obj_key, object in list(self.objects.items()):
                ...
                is_targeted = any(
                    arm.get('target_info') and arm['target_info'].get('target_object_id') == obj_id
                    for arm in self.arms
                )
                ...
                # only remove when confirmed gone - past belt end or in bin (dont remove if arm is targeting it)
                if not is_held and not is_targeted and (obj_pos[0] > self.config.conveyor_end_x or obj_pos[2] < 0.8):
    ...
    def _handle_wait_for_object(self, arm, sim_time):
        ...
        classes = arm['last_results'][0].boxes.cls.cpu().numpy() if len(boxes) > 0 else []
        
        if len(boxes) == 0:
            return
        
        # pick first trash detection above threshold (not just highest conf - that could be recycling)
        idx = None
        for i in np.argsort(confs)[::-1]:
            if confs[i] < self.config.confidence_threshold:
                continue
            class_name = self.sim_env.model.names[int(classes[i])] if hasattr(self.sim_env.model, 'names') and int(classes[i]) < len(self.sim_env.model.names) else str(int(classes[i]))
            if class_name in self.config.trash_classes:
                idx = i
                break
            logger.debug(f"[ARM {arm['id']} DETECT] '{class_name}' recyclable, skip")
        ...
        class_name = self.sim_env.model.names[int(classes[idx])] if hasattr(self.sim_env.model, 'names') and int(classes[idx]) < len(self.sim_env.model.names) else str(int(classes[idx]))
        ...
        if detected_world_pos[0] > arm_detection_line_x:
            # match detection to object for grab
            target_obj_id = None
            best_dist = float('inf')
            det_xy = (detected_world_pos[0], detected_world_pos[1])
            for obj_key, obj_data in self.objects.items():
                obj_id = obj_data[0]
                obj_pos = obj_data[1]
                d = ((obj_pos[0] - det_xy[0]) ** 2 + (obj_pos[1] - det_xy[1]) ** 2) ** 0.5
                if d < best_dist:
                    best_dist = d
            ...
            arm['target_info'] = {
                ...
                "target_object_id": target_obj_id if best_dist < 1.2 else None
            }
    ...
    def _handle_picking(self, arm):
        ...
        if arm['arm_substate'] is None:
            ...
            if wait_for_arm_to_reach(arm['kuka_id'], pickup_pos, threshold=self.config.arm_threshold):
                # prefer target from detection when we have it, else grab closest (dont skip)
                target_obj_id = arm['target_info'].get('target_object_id') if arm['target_info'] else None
                grab_obj_id = None
                grab_obj_key = None
                ...
                for obj_key, obj_data in self.objects.items():
                    ...
                    if target_obj_id == obj_id and dist < 0.6:
                        grab_obj_id = obj_id
                        grab_obj_key = obj_key
                        break
                    elif dist < 0.5 and dist < closest_dist:
                        closest_dist = dist
                        grab_obj_id = obj_id
                        grab_obj_key = obj_key
                
                if grab_obj_id is not None:
                    logger.info(f"[ARM {arm['id']}] Grab ID {grab_obj_id}")
                    arm['constraint_id'] = grab_object(arm['kuka_id'], grab_obj_id)
                    arm['target_object_id'] = grab_obj_id
                    arm['grabbed_obj_key'] = grab_obj_key
                    arm['picked'] = True
                    arm['tracking'] = True
                    ...
                    logger.warning(f"[ARM {arm['id']}] No obj in range")
                    arm['arm_substate'] = None
                    arm['picking_start_step'] = None
                    arm['target_info'] = None
                    arm['state'] = ArmState.RESETTING
